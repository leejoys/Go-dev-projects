...
// ErrNodeExist -  ошибка при создании существующего в дереве значения
var ErrNodeExist = fmt.Errorf("Узел существует")
...
func (t *IntTree) Insert(key int) error {
	if t.Get(key) != nil {
		return ErrNodeExist
	}
	var other *IntNode
...
func (t *IntTree) Delete(key int) error {
	node := t.Get(key)
	if node == nil {
		return ErrWrongTreeKey
	}
...
func main() {
	...
	//вызов методов без проверок на ошибки, для компактности
	someTree.Print(nil)
	someTree.Insert(1)
	someTree.Insert(9)
	someTree.Insert(2)
	...
	// тест ошибок
	err := someTree.Insert(7)
	if err != nil {
		fmt.Println(err)
	}
	err = someTree.Delete(11)
	if err != nil {
		fmt.Println(err)
	}
}
